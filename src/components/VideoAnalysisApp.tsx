...
// Enhanced stream start function
...
// ROI handlers with improved precision
...
// Enhanced frame processing with high-quality settings
const processFrame = () => {
  // Process first video with improved quality
  if (videoRef.current && canvasRef.current && isStreaming) {
    ...
    if (ctx) {
      ...
      // Draw the frame with high quality settings
      ctx.drawImage(
        videoRef.current,
        0, 0,
  ...
  // Process second video with improved quality
...
// Animation frame effect with cleanup
useEffect(() => {
  if (isStreaming || isStream2Active) {
    const frameId = requestAnimationFrame(processFrame);
    setAnimationFrame(frameId);
...
// Cleanup effect for camera resources
useEffect(() => {
  const cleanup = () => {
